...
void DemoApplication::init()
{
	...
	_scene_camera.position = { 0.0f, 0.0f, -50.0f };
	_scene_camera.look_at = glm::lookAt(_scene_camera.position, glm::vec3(0.0f, 0.0f, 0.0f),  glm::vec3(0.0f, 1.0f, 0.0f));
	_scene_camera.projection = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f);
	...
	_nolit_material = std::make_unique<Material>(nolit_vertex_shader,
		...
		.push_constants_updater = [&](Mesh* mesh, PushConstantsUpdater& updater) {
			...
			data.model_view = _scene_camera.look_at * glm::translate(glm::mat4_cast(_mesh_rotations[mesh->getId()]), _mesh_positions[mesh->getId()]);
			data.projection = _scene_camera.projection;
			
			const std::span<const uint8_t> data_view(reinterpret_cast<const uint8_t*>(&data), sizeof(data));
			updater.update(0, data_view);
		},
		}, 0
		);
	...
	_window->getRendererAs<UIRenderer>(UIRenderer::kRendererId).setOnGui(
		[&] {
			...
			_nolit_shader_controller.color_offset = value; 
			{
				...
				ImGui::SliderFloat("Rotation x", &rotation.x, 0.0, glm::pi<float>() * 2.0f);
				ImGui::SliderFloat("Rotation y", &rotation.y, 0.0, glm::pi<float>() * 2.0f);
				ImGui::SliderFloat("Rotation z", &rotation.z, 0.0, glm::pi<float>() * 2.0f);
