	...
	{
		{
			...
			_vertex_buffer = _window.gpuResourceManager().createAttributeBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, size);
		...
		}
		{
			...
			_index_buffer = _window.gpuResourceManager().createAttributeBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, size);
		...
		{
			frame_data.color_offset = _window.gpuResourceManager().createUniformBuffer(sizeof(ColorOffset));
